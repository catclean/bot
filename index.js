...
const replyMessages = JSON.parse(fs.readFileSync('./db/reply-messages.json', 'utf-8'));
const clientMessagesData = JSON.parse(fs.readFileSync('./db/client-message-data.json', 'utf-8'));
...
client.on('message', async(message) => {
    ...
    if (type == 'chat') {
        if (chatId == from) {
            if (body == '1') {client.sendMessage(from, replyMessages.adm)}
            if (body == '2') {client.sendMessage(from, replyMessages.list)}
            if (body == '3') {client.sendMessage(from, replyMessages.promo)}
            if (body == '4') {client.sendMessage(from, replyMessages.form)}
            if (body == '5') {client.sendMessage(from, replyMessages.address)}
            if (body == '6') {client.sendMessage(from, replyMessages.term)}
        }else{
